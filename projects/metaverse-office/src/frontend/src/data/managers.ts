...
export const ROLE_POSITIONS_V3: Record<ManagerRole | 'developer_east' | 'developer_west', [number, number, number] | [number, number, number][]> = {
  // 南房（前）- 市场部：市场专员（可多个实例）
  marketing: [
    [0, 0, 12],    // 中央
    [3, 0, 12],    // 偏右
  ...
  developer_east: [  // 东房研发（靠近方案部）
    [12, 0, -3],
    [12, 0, 3]
  ],
  
  // 西厢房（左）- 交付部：研发专家（后端）+ 交付运维
  developer_west: [  // 西房研发（靠近交付部）
    [-12, 0, -3],
    [-12, 0, 3]
  ],
  devops: [
...
export function getAgentPositionV3(
  ...
  // 特殊处理研发Agent的东西房分配
  if (role === 'developer') {
    const key = assignment === 'east' ? 'developer_east' : 'developer_west'
    const positions = ROLE_POSITIONS_V3[key]
    if (Array.isArray(positions[0])) {
      return (positions as [number, number, number][])[agentIndex % positions.length]
    }
